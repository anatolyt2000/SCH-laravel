...
//				}
	...
	public function export(){
		...
		foreach ($student as $value) {
			...
			$data[] = array ($value->studentRollId, $value->firstName.' '.$value->familyName,$value->gender,$value->address,$value->phoneNo,$value->mobileNo,$birthday,isset($classArray[$value->studentClass]) ? $classArray[$value->studentClass] : "",isset($sectionsArray[$value->studentSection]) ? $sectionsArray[$value->studentSection] : "", $countryTitle);
	...
	public function exportpdf(){
		...
		foreach ($student as $value) {
			...
			$data[] = array ($value->studentRollId, $value->firstName.' '.$value->familyName,$value->gender,$value->address,$value->mobileNo, isset($classArray[$value->studentClass]) ? $classArray[$value->studentClass] : "", $countryTitle);
		}
...
//		}
		...
		if(\Input::has('activeYear')){

			$ActiveYears=\Input::get('activeYear');
			$User->studentAcademicYear=$ActiveYears;
			$User->save();
			$AcademicYearsUpdate = \student_academic_years::where('studentId',$id)->first();
			$AcademicYearsUpdate->academicYearId = $ActiveYears;
			$AcademicYearsUpdate->save();
